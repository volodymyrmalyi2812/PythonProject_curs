...
class UserTestCase(unittest.TestCase):
    ...
    def test_str(self):
        """
        Перевіряємо метод __str__, чи повертає він коректний тип та значення.
        """
    ...
    def test_send_mail(self, mocked_send_mail):
        """
        Патчим метод відправки пошти, щоб у момент тестування не відбувалося реальної відправки.
        Але як правило треба перевіряти виклик, тобто ми запевнюємося що лист відправляється/не відправляється
        за викликом нашого методу `send_mail`.
        """
        self.user.send_mail()
